...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"Epoch {epoch:02d}/{EPOCHS} | loss={loss:.4f} | val Macro-F1={val_f1:.4f}")
    ...
    valid_out = pd.DataFrame({"id": valid_df["id"], "label": valid_preds})
    valid_out.to_csv("lstm_valid_pred.csv", index=False)
    print("Saved lstm_valid_pred.csv")
    ...
    test_out = pd.DataFrame({"id": test_df["id"], "label": test_preds})
    test_out.to_csv("lstm_pred.csv", index=False)
    print("Saved lstm_pred.csv")
